...
export class TypeInferencer {
  ...
  private inferNodeType(node: ExprNode): TypeResult | null {
    switch (node.kind) {
      ...
      case 'field':
        return this.inferFieldType(node.path);

      case 'call':
        return this.inferCallType(node.func, node.args);

      case 'method':
        return this.inferMethodType(node.receiver, node.method, node.args);

      case 'index':
        return this.inferIndexType(node.target, node.index);

      case 'slice':
        return this.inferSliceType(node.target, node.low, node.high, node.max);

      case 'pipeline':
        return this.inferPipelineType(node.stages);

      case 'binary':
        return this.inferBinaryType(node.op, node.left, node.right);

      case 'unary':
        return this.inferUnaryType(node.op, node.arg);
